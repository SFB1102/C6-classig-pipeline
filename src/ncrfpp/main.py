...
# THIS IS A MODIFIED VERSION OF THE ORIGINAL MAIN FILE
# TO BE USED WITH THE CLASSIG ANNOTATION PIPELINE
...
def decode(data, model, name):
    ...
    if data.nbest and not data.sentence_classification:
        data.write_nbest_decoded_results(pred_results, pred_scores, 'raw')

def load_data_model(config = "./ncrfpp/tmp/decode.config"):
    data = Data()
    data.HP_gpu = torch.cuda.is_available()
    data.read_config(config)
    load_model_dir = data.load_model_dir
    data.load(data.dset_dir)
    data.load_model_dir = load_model_dir
    model = load_model(data)
    return data, model

def chunk(data, model, config = "./ncrfpp/tmp/decode.config"):
    data.read_config(config)
    decode(data, model, "raw")
